 ...
 */
function isAdExpired(ad: AdModel): boolean {
  // Ad is expired if:
  // 1. Not active
  ...
  const expiryTime = new Date(
    startTime.getTime() + ad.durationHours * 60 * 60 * 1000
  );
  const now = new Date();
 ...
 */
export async function deleteExpiredAdsImages(
  ...
  try {
    ...
    if (updatedCount > 0) {
      ...
      logger.info(
        `deleteExpiredAdsImages: Updated ${updatedCount} ads in Firestore`
      );
    }
